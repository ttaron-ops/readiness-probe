...
- **Google, "Training in Turmoil: Silent Data Corruption in Systems at Scale"** — <https://research.google/pubs/training-in-turmoil-silent-data-corruption-in-systems-at-scale/> — SDC at Gemini training scale: events observed roughly **every one to two weeks** at ~10,000 chips, detected via **deterministic-replay** (rerun the suspect step and compare bitwise) plus **proactive scanners** on idle machines, and **elastic** recovery continuing at ~97 % of prior throughput on fewer chips. *What it shows:* §11's detection strategy is what a hyperscaler actually runs, not a proposal.
- **Meta, "The Llama 3 Herd of Models"** — <https://arxiv.org/abs/2407.21783> — over a 54-day, ~16K-H100 run: **466 job interruptions** (419 unexpected, 47 planned), **> 90 % effective training time**, and ~**78 % of unexpected interruptions** attributed to hardware, six specifically to silent data corruption. *What it shows:* §12's job-level MTBF arithmetic is not pessimism, it is what a real 16K-GPU run measures.
- **Slack, "Slack's Incident on 2-22-22"** — <https://slack.engineering/slacks-incident-on-2-22-22/> — a load spike on a Vitess keyspace triggered a retry storm that sustained the overload even though the clients were **already using exponential backoff with jitter**. *What it shows:* the empirical case for retry *budgets* over retry *backoff* — per-client correctness says nothing about the aggregate. Recovery was shed-don't-defer: throttle at the edge, fix the amplifier, then let traffic back in.
- **Huang, L. et al., "Metastable Failures in the Wild" (OSDI '22)** — <https://www.usenix.org/conference/osdi22/presentation/huang-lexiang> — **at least 4 of the last 15 major AWS outages** analysed followed the metastable pattern; the study covers **22 incidents across 11 organizations**. *What it shows:* §9's two-threshold hysteresis model describes how large systems actually fail.
- **Roblox, "Roblox Return to Service 10/28–10/31 2021"** — <https://blog.roblox.com/2022/01/roblox-return-to-service-10-28-10-31-2021/> — a 73-hour outage rooted in Consul's use of BoltDB under a specific load pattern, whose diagnosis was crippled because the monitoring depended on the failing system. *What it shows:* differential observability applied to the *operator's* vantage — an observer inside the failure domain disappears exactly when you need it.
- **PagerDuty, "August 28 Kafka Outages"** — <https://www.pagerduty.com/eng/august-28-kafka-outages-what-happened-and-how-were-improving/> — a producer-per-request bug flooded the broker fleet, and the recovery produced **duplicate webhooks** as the backlog drained. *What it shows:* the recovery path is where delivery-semantics bugs surface — the bridge into lesson 07.
